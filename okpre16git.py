...
if st.session_state.get('user_info') is not None:
    ...
    # --- Vision API ラベル抽出関数 ---
    # <<< 全文: get_vision_labels_from_uploaded_images関数 >>># --- Vision API ラベル抽出関数 ---
# <<< 全文: get_vision_labels_from_uploaded_images関数 (Secrets対応) >>>
    def get_vision_labels_from_uploaded_images(image_files):
        """画像からVision APIでラベル抽出 (Secrets対応版)"""
        if not vision or not service_account or not Request: st.warning("Visionライブラリ未検出"); return []
        
        try:
        # SecretsからFirebase SAキーJSON文字列を取得 (Visionも同じキーを使うと仮定)
        # もし別のキーを使う場合はSecretsのキー名を変える
            service_account_json_str = st.secrets.get("FIREBASE_SERVICE_ACCOUNT_JSON")
            if not service_account_json_str:
                st.warning("Vision用認証情報がSecretsに未設定 (FIREBASE_SERVICE_ACCOUNT_JSON)")
                return []
            # JSON文字列を辞書に変換
            service_account_info = json.loads(service_account_json_str)

        # 辞書からCredentialsオブジェクトを作成
            ...
            st.error("SecretsのVision用認証情報 (JSON) の形式が正しくありません。")
            return []
        except Exception as e:
            st.error(f"Vision API全体エラー (Secrets読み込み含む): {e}")
            print(f"Vision overall err: {e}")
